...
"""
#faire une liste de dict : i=dic_for_algo_i
global word2oseg
...
def find_oseg(wordg,oseg_occ,output,word2oseg,word2oseg_occ):
    #count=0
    #print("word :",wordg,"occ:",j[0]["over"][wordg])
    before_cut = wordg[0]
        ...
        for sent in output:
            ...
            if seg!=None :
                ...
                if seg not in word2oseg[wordg]:
                    ...
                    #word2oseg[wordg].append(1)
                else :
                    for elt in word2oseg_occ[wordg]:
                        if elt[0]==seg:
                            elt[1]+=1
                        #word2oseg[wordg][-1]+=1
                    #print("sent",sent,"word_i",wordg,"seg",seg)
    #print("occ_find : ",count)
...
def analyze_sent(sent,bc,w):
    ...
    if bc in sent : 
        #print(seg_form)
        #print(sent)
        #jusquici ca va mais ne rpend pas la 2e partie du mt : faire gaffe a l'espace ??
...
"""def return_child_dic():
    for i in range(0,len(j)):
        store_word_oseg(j[i]["over"],o[i])
    return(word2oseg_occ)
"""
...
path_name = child+'_datas.json'
with open(path_name,'w') as file:
    json.dump(listdics,file)
print(type(listdics[3]))
print(len(listdics))
print(len(listdics[3]))
